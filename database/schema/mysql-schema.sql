...
CREATE TABLE `fee_vouchers` (
  ...
  `voucher_type` enum('monthly','custom','multiple') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'monthly',
  `custom_amount` decimal(10,2) DEFAULT NULL,
  `fee_month` varchar(255) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Format: YYYY-MM',
  `fee_breakdown` json DEFAULT NULL COMMENT 'Store multiple fee types breakdown',
  `selected_fee_types` json DEFAULT NULL COMMENT 'Store selected fee type IDs',
  `notes` text COLLATE utf8mb4_unicode_ci,
  `status` enum('paid','unpaid','partially_paid','cancelled') COLLATE utf8mb4_unicode_ci DEFAULT 'unpaid' COMMENT 'Voucher payment status: paid (fully paid), unpaid (not paid), partially_paid (partial payment received), cancelled (voucher cancelled)',
  ...
  FULLTEXT KEY `ft_student_parent_name` (`student_name`,`parent_name`),
  FULLTEXT KEY `ft_student_name` (`student_name`),
  FULLTEXT KEY `ft_parent_name` (`parent_name`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
