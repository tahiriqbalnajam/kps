<template>
  <div>
    <el-row :gutter="16" class="filter-row">
      ...
      <el-col :span="5">
        <el-select v-model="voucherType" placeholder="All Types" clearable @change="fetchReport" class="w-full">
    ...
    <el-row :gutter="16" class="summary-row">
      ...
      <el-col :span="6">
        <el-card shadow="never" class="summary-card">
          <div class="stat-value">{{ formatCurrency(summary.total_fee_amount) }}</div>
          <div class="stat-label">Total Fee Amount</div>
        </el-card>
      </el-col>
      <el-col :span="6">
        <el-card shadow="never" class="summary-card">
          <div class="stat-value">{{ formatCurrency(summary.total_fine_amount) }}</div>
    ...
    <el-table :data="records" border stripe v-loading="loading" class="data-table">
      ...
      <el-table-column prop="fee_amount" label="Fee" width="100" />
      <el-table-column prop="fine_amount" label="Fine" width="80" />
...
const fetchReport = async () => {
  loading.value = true
  try {
    const params = { type: 1, per_page: perPage.value, page: currentPage.value }
    if (dateRange.value?.length === 2) {
      params.date_from = dateRange.value[0]
      params.date_to = dateRange.value[1]
    }
    if (className.value) params.class_name = className.value
    if (voucherType.value) params.voucher_type = voucherType.value
